 ...
 *
 * Unlike [Iso], [Equiv] is itself a typeclass and is intended to be synthesized by the compiler
 * rather than authored directly by user code. Exported `Equiv` evidence participates in typeclass
 * search; internal proof trees that only exist while completing one derivation do not necessarily
 * become globally summonable.
 *
...
public abstract class Equiv<A, B> protected constructor() {
    ...
    public fun inverse(): Equiv<B, A> = unsafeEquiv(to = ::from, from = ::to)
 ...
 *
 * This exists primarily for compiler-synthesized evidence and library internals. Callers are
 * responsible for ensuring that [to] and [from] really form a reversible equivalence.
 */
@InternalTypeclassApi
public fun <A, B> unsafeEquiv(to: (A) -> B, from: (B) -> A): Equiv<A, B> =
    object : Equiv<A, B>() {
