 ...
 *
 * These notions are intentionally distinct from builtin [kotlin.reflect.KClass].
 * `KClass` is runtime classifier identity only: it erases type arguments and does
 * not model nullability. [KnownType] and [TypeId] are the exact and semantic
 * identity utilities respectively.
 *
 * In particular, [KnownType] should be read as "the compiler knows the exact
 * instantiated type here", not "the compiler knows something about a source-level
 * type parameter symbol". A reified `T` instantiated as `String` should behave the
 * same as a concrete `String` for [KnownType].
 */

/**
 * Witnesses semantic Kotlin subtyping.
 *
 * A value of `Subtype<Sub, Super>` means that any value of `Sub` may be used where
 * `Super` is expected.
 */
@Typeclass
public interface Subtype<Sub, Super> {
    ...
    public fun coerce(value: Sub): Super = value as Super

    /**
     * Composes two subtype proofs transitively.
     */
    public fun <Upper> andThen(that: Subtype<Super, Upper>): Subtype<Sub, Upper> = unsafeAssertSubtype()

    /**
     * Composes two subtype proofs transitively.
     */
    public fun <Lower> compose(that: Subtype<Lower, Sub>): Subtype<Lower, Super> = unsafeAssertSubtype()
    ...
    public companion object {
         ...
         */
        public fun <Sub, Super> reify(): Subtype<Sub, Super>
            where Sub : Super = unsafeAssertSubtype()
    }
}
 ...
 *
 * `Same<A, B>` should line up with the compiler's notion of identical expanded
 * Kotlin types. Type aliases do not create distinct [Same] proofs.
 */
@Typeclass
public interface Same<A, B> : Subtype<A, B> {
    ...
    public companion object {
         ...
         */
        public fun <A, B> bracket(
            left: Subtype<A, B>,
            right: Subtype<B, A>,
        ): Same<A, B> = unsafeAssertSame()
    }
}
 ...
 *
 * This is the lightweight, proof-only notion of inequality. Unlike a future
 * stronger apartness proof, [NotSame] does not carry a canonical identity token.
 */
@Typeclass
public interface NotSame<A, B> {
    ...
    /**
     * Reaching both equality and inequality proofs at the same time is
     * contradictory.
     */
    ...
    public companion object {
        /**
         * Builds an inequality proof from a contradiction that would arise if
         * equality were ever provided.
         */
        public fun <A, B> fromContradiction(
            contradiction: (Same<A, B>) -> Nothing,
        ): NotSame<A, B> =
            object : NotSame<A, B> {
         ...
         */
        public fun <A> irreflexive(proof: NotSame<A, A>): Nothing =
            proof.contradicts(Same.refl())
    }
}
...
/**
 * Witnesses proper subtyping: `Sub` is a subtype of `Super`, but the two are not
 * equal.
 */
@Typeclass
public interface StrictSubtype<Sub, Super> : Subtype<Sub, Super>, NotSame<Sub, Super> {
     ...
     */
    override fun <Upper> andThen(that: Subtype<Super, Upper>): StrictSubtype<Sub, Upper> = unsafeAssertStrictSubtype()

    /**
     * Proper subtyping composes transitively with ordinary subtyping.
     */
    override fun <Lower> compose(that: Subtype<Lower, Sub>): StrictSubtype<Lower, Super> = unsafeAssertStrictSubtype()
 ...
 *
 * This is stronger than "the type is nullable syntax-wise". The intended
 * semantics are "the compiler can prove that `null` is a valid value of `T`".
 * For an unconstrained type parameter, neither [Nullable] nor [NotNullable]
 * should materialize.
 */
@Typeclass
public interface Nullable<T> {
     ...
     */
    @Suppress("UNCHECKED_CAST")
    public fun nullValue(): T = null as T
    ...
    /**
     * Reaching both nullable and non-nullable proofs at the same time is
     * contradictory.
     */
 ...
 *
 * This should be read as "the compiler can prove `T <: Any`", not merely
 * "the type was written without a trailing `?`".
 */
@Typeclass
public interface NotNullable<T> {
    ...
    /**
     * If `Lower` is a subtype of `T` and `T` excludes null, then `Lower` also
     * excludes null.
     */
    public fun <Lower> compose(that: Subtype<Lower, T>): NotNullable<Lower> = unsafeAssertNotNullable()
 ...
 * - should not preserve source-only alias names or declaration-site type
 *   parameter identity as part of its public semantics
 *
 * [KnownType] is intentionally richer than builtin [kotlin.reflect.KClass], which
 * only captures runtime classifier identity.
 */
@Typeclass
...
private data class KnownTypeImpl(
 ...
 */
public fun knownType(
    kType: KType,
): KnownType<Any?> = KnownTypeImpl(kType)
 ...
 * - `left.sameAs(right)` should hold exactly when the compiler could prove
 *   `Same<A, B>` for the corresponding type arguments
 * - `left == right` and `left.hashCode() == right.hashCode()` follow the same
 *   semantic identity relation
 * - it preserves classifier, type arguments, and nullability
 * - compiler-synthesized ids should not treat source alias names as
 *   semantically meaningful distinctions
 * - reflective fallback ids may still preserve source type-parameter names when
 *   the runtime [KType] is not fully concrete
 *
 * Unlike [KnownType], [TypeId] is meant for fast equality checks, hashing, and
 * durable map/set keys. A [TypeId] should therefore be safe to use as a key in
 * a `HashMap` or member of a `HashSet` without risking mismatches between
 * semantic comparison and runtime equality. Unlike builtin [kotlin.reflect.KClass],
 * it is not erased.
 */
@Typeclass
public class TypeId<T> private constructor(
    /**
     * A canonical semantic rendering used for equality and diagnostics.
     *
     * This is part of the public contract for tests and debugging, but callers
     * should prefer [sameAs] or [compare] for identity checks.
     */
    ...
    /**
     * Compares two semantic type ids and returns either equality or inequality
     * evidence.
     */
    ...
    public companion object {
         ...
         *
         * Compiler-synthesized [TypeId] values should use a canonical rendering
         * whose equality matches [Same]. Reflective fallback ids preserve the
         * semantics of [typeId(kType)] below and may therefore retain reflective
         * type-parameter names for non-concrete [KType] values.
         */
        public fun <T> fromCanonicalName(canonicalName: String): TypeId<T> =
            TypeId(canonicalName)

        /**
         * Compares two semantic type identifiers.
         */
        public fun <A, B> compare(
            left: TypeId<A>,
            right: TypeId<B>,
        ): TypeIdComparison<A, B> = left.compare(right)
    }
}
 ...
 * Wraps a canonical semantic name into a [TypeId].
 *
 * Compiler-synthesized [TypeId] values should use a canonical rendering whose
 * equality matches [Same]. Reflective fallback ids preserve the semantics of
 * [typeId(kType)] below and may therefore retain reflective type-parameter
 * names for non-concrete [KType] values.
 */
public fun typeId(
    canonicalName: String,
): TypeId<Any?> = TypeId.fromCanonicalName(canonicalName)

/**
 * Wraps an exact [KType] into a semantic [TypeId].
 *
 * This is primarily used by compiler-synthesized evidence for reified types.
 * For non-concrete reflective [KType] values, the resulting canonical name may
 * still retain reflective type-parameter names.
 */
public fun typeId(
    kType: KType,
): TypeId<Any?> = TypeId.fromCanonicalName(kType.toCanonicalTypeIdName())
...
@InternalTypeclassApi
@Suppress("UNCHECKED_CAST")
private fun <Sub, Super> unsafeAssertSubtype(): Subtype<Sub, Super> = UnsafeAssertSubtype as Subtype<Sub, Super>
...
@InternalTypeclassApi
@Suppress("UNCHECKED_CAST")
private fun <T> unsafeAssertNotNullable(): NotNullable<T> = UnsafeAssertNotNullable as NotNullable<T>
...
private fun KType.toCanonicalTypeIdName(): String =
    buildString {
        append(classifierCanonicalName())
        if (arguments.isNotEmpty()) {
            append(arguments.joinToString(prefix = "<", postfix = ">", separator = ",") { argument ->
