 ...
 * @property instanceSlot compiler-managed storage that resolves to the field's required typeclass
 *   instance, potentially through a recursive cell
 ...
 * @property isValueClass whether the product is represented as a Kotlin value class
 ...
 * @property instanceSlot compiler-managed storage that resolves to the case's typeclass instance,
 *   potentially through a recursive cell
 * @property matcher function that tests whether a runtime value belongs to this case
 */
 ...
 */
public data class SumTypeclassMetadata(val typeName: String, val cases: List<SumCaseMetadata>)
 ...
 */
public data class EnumEntryMetadata(val name: String)
 ...
 * @property ordinalResolver compiler-managed bridge that extracts an ordinal from a runtime enum
 *   value
 * @property valueResolver compiler-managed bridge that reconstructs a value by ordinal
 */
public class EnumTypeclassMetadata(
    ...
    public fun valueAt(index: Int): Any? {
        require(index in entries.indices) { "Enum index $index is out of range for $typeName" }
        return valueResolver(index)
            ?: error("Enum value resolver returned null for $typeName[$index]")
    }
    ...
    public fun construct(name: String): Any? {
        ...
        require(index >= 0) { "Unknown enum entry $name for $typeName" }
...
/**
 * Derivation hook for typeclasses that can be synthesized for products, sealed sums, and optionally
 * enums.
 */
 ...
 *
 * During recursive derivation the compiler may publish a placeholder cell first and fill [value]
 * once the final instance has been constructed.
 */
public class RecursiveTypeclassInstanceCell(public var value: Any? = null)

private fun resolveTypeclassInstance(instanceSlot: Any): Any =
    when (instanceSlot) {
        is RecursiveTypeclassInstanceCell ->
            instanceSlot.value
                ?: error("Recursive typeclass instance accessed before initialization")
