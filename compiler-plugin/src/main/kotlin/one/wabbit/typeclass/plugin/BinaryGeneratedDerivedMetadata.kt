...
internal object BinaryGeneratedDerivedMetadataRegistry {
    private val loaders =
        Collections.synchronizedMap(WeakHashMap<FirSession, BinaryGeneratedDerivedMetadataLoader>())

    fun install(session: FirSession, loader: BinaryGeneratedDerivedMetadataLoader) {
        synchronized(loaders) { loaders.getOrPut(session) { loader } }
    }
...
internal class BinaryGeneratedDerivedMetadataLoader(private val classpathRoots: List<File>) :
    AutoCloseable {
    ...
    override fun close() {
        metadataRoots.forEach { root -> runCatching { root.close() } }
    }
}
...
internal sealed interface BinaryGeneratedDerivedMetadataLookupResult {
    ...
    data class Found(val metadata: List<GeneratedDerivedMetadata>) :
...
internal class DirectoryBinaryGeneratedDerivedMetadataRoot(private val root: File) :
    BinaryGeneratedDerivedMetadataRoot {
    override fun generatedMetadataFor(
        classId: ClassId
    ): BinaryGeneratedDerivedMetadataLookupResult {
...
internal class JarBinaryGeneratedDerivedMetadataRoot(
    ...
    private val lookupByEntryPath =
        ConcurrentHashMap<String, BinaryGeneratedDerivedMetadataLookupResult>()
    ...
    override fun generatedMetadataFor(
        ...
        return lookupByEntryPath.computeIfAbsent(entryPath) {
            lookupEntry(entryPath = entryPath, ownerId = classId.asString())
        }
    }
...
private fun binaryGeneratedDerivedMetadataRoot(root: File): BinaryGeneratedDerivedMetadataRoot? =
    when {
        ...
        root.isFile && root.extension.equals("jar", ignoreCase = true) ->
            JarBinaryGeneratedDerivedMetadataRoot(root)
        else -> null
    }
            ...
            object : ClassVisitor(Opcodes.ASM9) {
                override fun visitAnnotation(
                    ...
                    when (descriptor) {
                        ...
                        containerDescriptor ->
                            generatedMetadataContainerVisitor(entries, ownerId, generatedDescriptor)
                        else -> null
                    }
            },
            ClassReader.SKIP_CODE or ClassReader.SKIP_DEBUG or ClassReader.SKIP_FRAMES,
        )
    return entries.takeIf(List<GeneratedDerivedMetadata>::isNotEmpty) ?: emptyList()
}
...
private fun parseGeneratedDerivedMetadataOrEmpty(
    ...
    try {
        parseGeneratedDerivedMetadata(classBytes = classBytes, ownerId = ownerId)
