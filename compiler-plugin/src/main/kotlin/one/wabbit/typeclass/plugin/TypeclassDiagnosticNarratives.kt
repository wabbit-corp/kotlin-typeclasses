...
private val genericWhyFixRegex = Regex("""^(.*)\s+Why it failed:\s+(.*)\s+How to fix:\s+(.*)$""")
private val directRecursiveRegex =
    Regex("""^direct recursive instance rule for (.+) is not allowed\.$""")
private val topLevelOrphanRegex =
    Regex(
        """^top-level orphan instance declarations must be declared in the same file as one of: (.+)\.$"""
    )
private val typeParameterPrereqRegex =
    Regex(
        """^instance type parameter (.+) must appear in the provided type, not only prerequisites\.$"""
    )
private val constructiveMismatchRegex =
    Regex(
        """^Cannot derive (.+) because constructive product derivation requires constructor parameters to exactly match stored properties\.$"""
    )
private val wrongDeriverReturnRegex =
    Regex(
        """^(deriveProduct|deriveSum|deriveEnum) must return ([^<]+)<\.\.\.>(?:; found (.+))?\.$"""
    )
...
sealed interface TypeclassDiagnostic {
    ...
    data class AmbiguousInstance(
        ...
        init {
            require(candidates.isNotEmpty()) {
                "Ambiguous diagnostics must list at least one candidate"
            }
        }
    }
    ...
    sealed interface CannotDeriveCase {
        ...
        data class ConstructiveProductStoredPropertyMismatch(val typeName: String) :
            CannotDeriveCase
...
internal fun TypeclassDiagnostic.render(): String =
    TypeclassDiagnosticIds.format(diagnosticId, renderBody())
...
internal val TypeclassDiagnostic.headline: String
    get() =
        when (this) {
            ...
            is TypeclassDiagnostic.InvalidBuiltinEvidence -> renderSections(this).first
        }
...
internal fun parseTypeclassDiagnostic(
    ...
    val id =
        effectiveDiagnosticId ?: inferDiagnosticIdFromHeadline(sections.headline) ?: return null
    return when (id) {
        ...
        TypeclassDiagnosticIds.CANNOT_DERIVE ->
            TypeclassDiagnostic.CannotDerive(parseCannotDeriveCase(sections.reason, sections.fix))
        TypeclassDiagnosticIds.INVALID_INSTANCE_DECL ->
            TypeclassDiagnostic.InvalidInstance(
                parseInvalidInstanceCase(sections.reason, sections.fix)
            )
        TypeclassDiagnosticIds.INVALID_EQUIV_DECL ->
            TypeclassDiagnostic.InvalidEquiv(parseInvalidEquivCase(sections.reason, sections.fix))
...
internal fun cannotDeriveConstructiveProductStoredPropertyMismatch(
    typeName: String
): TypeclassDiagnostic =
    TypeclassDiagnostic.CannotDerive(
        TypeclassDiagnostic.CannotDeriveCase.ConstructiveProductStoredPropertyMismatch(typeName)
    )
...
internal fun cannotDeriveMissingEnumOverride(typeclassName: String): TypeclassDiagnostic =
    TypeclassDiagnostic.CannotDerive(
        TypeclassDiagnostic.CannotDeriveCase.MissingEnumOverride(typeclassName)
    )

internal fun cannotDeriveDiagnostic(reason: String): TypeclassDiagnostic =
    TypeclassDiagnostic.CannotDerive(
        parseCannotDeriveCase(
            reason.ensureSentence(),
            fallbackCannotDeriveFix(reason.ensureSentence()),
        )
    )
...
internal fun invalidInstanceCustomGetterProperty(): TypeclassDiagnostic =
    TypeclassDiagnostic.InvalidInstance(
        TypeclassDiagnostic.InvalidInstanceCase.CustomGetterProperty()
    )

internal fun invalidInstanceNonTypeclassSupertypes(): TypeclassDiagnostic =
    TypeclassDiagnostic.InvalidInstance(
        TypeclassDiagnostic.InvalidInstanceCase.NonTypeclassSupertypes()
    )

internal fun invalidInstanceMustProvideTypeclassType(): TypeclassDiagnostic =
    TypeclassDiagnostic.InvalidInstance(
        TypeclassDiagnostic.InvalidInstanceCase.MustProvideTypeclassType()
    )

internal fun invalidInstanceTopLevelOrphan(hostNames: List<String>): TypeclassDiagnostic =
    TypeclassDiagnostic.InvalidInstance(
        TypeclassDiagnostic.InvalidInstanceCase.TopLevelOrphan(hostNames)
    )

internal fun invalidInstanceWrongScopeCompanion(): TypeclassDiagnostic =
    TypeclassDiagnostic.InvalidInstance(
        TypeclassDiagnostic.InvalidInstanceCase.WrongScopeCompanion()
    )

internal fun invalidInstanceAssociatedOwnerMismatch(): TypeclassDiagnostic =
    TypeclassDiagnostic.InvalidInstance(
        TypeclassDiagnostic.InvalidInstanceCase.AssociatedOwnerMismatch()
    )

internal fun invalidInstanceNonTypeclassPrerequisites(): TypeclassDiagnostic =
    TypeclassDiagnostic.InvalidInstance(
        TypeclassDiagnostic.InvalidInstanceCase.NonTypeclassPrerequisites()
    )

internal fun invalidInstanceStarProjectedPrerequisites(): TypeclassDiagnostic =
    TypeclassDiagnostic.InvalidInstance(
        TypeclassDiagnostic.InvalidInstanceCase.StarProjectedPrerequisites()
    )

internal fun invalidInstanceDefinitelyNotNullPrerequisites(): TypeclassDiagnostic =
    TypeclassDiagnostic.InvalidInstance(
        TypeclassDiagnostic.InvalidInstanceCase.DefinitelyNotNullPrerequisites()
    )

internal fun invalidInstanceTypeParameterOnlyInPrerequisites(
    parameterName: String
): TypeclassDiagnostic =
    TypeclassDiagnostic.InvalidInstance(
        TypeclassDiagnostic.InvalidInstanceCase.TypeParameterOnlyInPrerequisites(parameterName)
    )

internal fun invalidInstanceDirectRecursive(goal: String): TypeclassDiagnostic =
    TypeclassDiagnostic.InvalidInstance(
        TypeclassDiagnostic.InvalidInstanceCase.DirectRecursive(goal)
    )

internal fun invalidInstanceDiagnostic(reason: String): TypeclassDiagnostic =
    TypeclassDiagnostic.InvalidInstance(
        parseInvalidInstanceCase(
            reason.ensureSentence(),
            fallbackInvalidInstanceFix(reason.ensureSentence()),
        )
    )
...
internal fun invalidEquivDiagnostic(reason: String): TypeclassDiagnostic =
    TypeclassDiagnostic.InvalidEquiv(
        parseInvalidEquivCase(
            reason.ensureSentence(),
            fallbackInvalidEquivFix(reason.ensureSentence()),
        )
    )
...
internal fun ambiguousTypeclassInstanceDiagnostic(
    goal: String,
    scope: String,
    candidates: List<String>,
): String =
    TypeclassDiagnostic.AmbiguousInstance(goal = goal, scope = scope, candidates = candidates)
        .renderBody()
...
internal fun enrichCannotDeriveDiagnostic(reason: String): String =
    cannotDeriveDiagnostic(reason).renderBody()

internal fun enrichInvalidInstanceDiagnostic(reason: String): String =
    invalidInstanceDiagnostic(reason).renderBody()

internal fun enrichInvalidEquivDiagnostic(reason: String): String =
    invalidEquivDiagnostic(reason).renderBody()

internal fun enrichInvalidBuiltinEvidenceDiagnostic(reason: String): String =
    invalidBuiltinEvidenceDiagnostic(reason).renderBody()

private fun renderCannotDeriveCase(
    case: TypeclassDiagnostic.CannotDeriveCase
...
private fun renderInvalidInstanceCase(
    case: TypeclassDiagnostic.InvalidInstanceCase
): Triple<String, String, String> =
    when (case) {
        is TypeclassDiagnostic.InvalidInstanceCase.ClassBased ->
            Triple(
                "Invalid @Instance declaration.",
                "class-based instances are not allowed; use an object.",
                "declare the instance as an object, a top-level @Instance function, or a val instead of a class.",
            )
        is TypeclassDiagnostic.InvalidInstanceCase.ExtensionFunction ->
            Triple(
                "Invalid @Instance declaration.",
                "extension instance functions are not allowed.",
                "drop the extension receiver and express dependencies through type parameters or context parameters instead.",
            )
        is TypeclassDiagnostic.InvalidInstanceCase.RegularParameter ->
            Triple(
                "Invalid @Instance declaration.",
                "instance functions cannot declare a regular parameter.",
                "remove regular value parameters from the @Instance function and model prerequisites as context parameters instead.",
            )
        is TypeclassDiagnostic.InvalidInstanceCase.SuspendFunction ->
            Triple(
                "Invalid @Instance declaration.",
                "suspend instance functions are not allowed.",
                "make the @Instance function eager and non-suspend, then call suspend helpers from inside the returned object if needed.",
            )
        is TypeclassDiagnostic.InvalidInstanceCase.ExtensionProperty ->
            Triple(
                "Invalid @Instance declaration.",
                "extension instance properties are not allowed.",
                "move the instance property to top level or into a companion, and express dependencies without an extension receiver.",
            )
        is TypeclassDiagnostic.InvalidInstanceCase.LateinitProperty ->
            Triple(
                "Invalid @Instance declaration.",
                "lateinit instance property declarations are not allowed.",
                "replace the declaration with an immutable val that is initialized directly at the declaration site.",
            )
        is TypeclassDiagnostic.InvalidInstanceCase.MutableProperty ->
            Triple(
                "Invalid @Instance declaration.",
                "mutable instance property declarations are not allowed.",
                "replace the declaration with an immutable val that is initialized directly at the declaration site.",
            )
        is TypeclassDiagnostic.InvalidInstanceCase.CustomGetterProperty ->
            Triple(
                "Invalid @Instance declaration.",
                "custom getter instance property declarations are not allowed.",
                "replace the declaration with an immutable val that is initialized directly at the declaration site.",
            )
        is TypeclassDiagnostic.InvalidInstanceCase.NonTypeclassSupertypes ->
            Triple(
                ...
                "remove the non-typeclass intermediate supertype or provide the typeclass head directly.",
            )
        is TypeclassDiagnostic.InvalidInstanceCase.MustProvideTypeclassType ->
            Triple(
                "Invalid @Instance declaration.",
                "@Instance declarations must provide a @Typeclass type.",
                "change the provided type so the declaration returns a real @Typeclass application.",
            )
        is TypeclassDiagnostic.InvalidInstanceCase.TopLevelOrphan ->
            Triple(
                "Invalid @Instance declaration.",
                "top-level orphan instance declarations must be declared in the same file as one of: ${case.hostNames.joinToString()}.",
                "move the declaration into the same file as one of the allowed host types, or into an allowed companion.",
            )
        is TypeclassDiagnostic.InvalidInstanceCase.WrongScopeCompanion ->
            Triple(
                ...
                "move the declaration to top level or into the companion of the provided typeclass head or one of its associated owners.",
            )
        is TypeclassDiagnostic.InvalidInstanceCase.AssociatedOwnerMismatch ->
            Triple(
                ...
                "move the declaration into the companion of the provided typeclass head or one of the associated type arguments.",
            )
        is TypeclassDiagnostic.InvalidInstanceCase.NonTypeclassPrerequisites ->
            Triple(
                ...
                "model prerequisites only as typeclass applications that the solver can satisfy directly.",
            )
        is TypeclassDiagnostic.InvalidInstanceCase.StarProjectedPrerequisites ->
            Triple(
                ...
                "keep @Instance prerequisites as ordinary non-star-projected typeclass applications that the solver can satisfy directly.",
            )
        ...
        is TypeclassDiagnostic.InvalidInstanceCase.TypeParameterOnlyInPrerequisites ->
            Triple(
                "Invalid @Instance declaration.",
                "instance type parameter ${case.parameterName} must appear in the provided type, not only prerequisites.",
                "make every instance type parameter contribute to the provided typeclass head instead of only its prerequisites.",
            )
        is TypeclassDiagnostic.InvalidInstanceCase.DirectRecursive ->
            Triple(
                "Invalid @Instance declaration.",
                "direct recursive instance rule for ${case.goal} is not allowed.",
                "break the direct self-reference or move the recursion into a supported derived rule.",
            )
        is TypeclassDiagnostic.InvalidInstanceCase.Generic ->
            Triple(
                "Invalid @Instance declaration.",
                case.reason.ensureSentence(),
                case.fix.ensureSentence(),
            )
    }

private fun renderInvalidEquivCase(
    case: TypeclassDiagnostic.InvalidEquivCase
): Triple<String, String, String> =
    when (case) {
        is TypeclassDiagnostic.InvalidEquivCase.Subclassing ->
            Triple(
                "Invalid Equiv declaration.",
                "Equiv is compiler-owned; users must not subclass it directly.",
                "use Iso for explicit user-authored reversible conversions, or @DeriveEquiv / @DeriveVia when the compiler should synthesize the evidence.",
            )
        is TypeclassDiagnostic.InvalidEquivCase.PublishedInstance ->
            Triple(
                "Invalid Equiv declaration.",
                "Equiv is compiler-owned and cannot be published as an @Instance.",
                "remove the Equiv @Instance and expose an Iso instead; Equiv is compiler-owned evidence.",
            )
...
private fun renderInvalidBuiltinEvidenceCase(
    case: TypeclassDiagnostic.InvalidBuiltinEvidenceCase
): Triple<String, String, String> =
    when (case) {
        is TypeclassDiagnostic.InvalidBuiltinEvidenceCase.Generic ->
            Triple(
                "Invalid builtin evidence request.",
                case.reason.ensureSentence(),
                case.fix.ensureSentence(),
            )
    }
...
private fun renderSections(diagnostic: TypeclassDiagnostic): Triple<String, String, String> =
    when (diagnostic) {
        ...
        is TypeclassDiagnostic.InvalidBuiltinEvidence -> {
            ...
            val sections =
                parseNarrativeSections(body)
                    ?: error("Failed to parse self-rendered diagnostic: $body")
...
private fun parseAmbiguousDiagnostic(sections: ParsedNarrativeSections): TypeclassDiagnostic? {
    ...
    val candidates =
        sections.reason
            .removePrefix("multiple candidates matched: ")
            .removeSuffix(".")
            .split(", ")
...
private fun parseCannotDeriveCase(
    reason: String,
    fix: String,
): TypeclassDiagnostic.CannotDeriveCase =
    when {
        ...
        missingEnumOverrideRegex.matches(reason) ->
            TypeclassDiagnostic.CannotDeriveCase.MissingEnumOverride(
                missingEnumOverrideRegex.matchEntire(reason)!!.groupValues[1]
            )
        else ->
            TypeclassDiagnostic.CannotDeriveCase.Generic(
                reason.ensureSentence(),
                fix.ensureSentence(),
            )
    }

private fun parseInvalidInstanceCase(
    reason: String,
    fix: String,
): TypeclassDiagnostic.InvalidInstanceCase =
    when {
        reason == "class-based instances are not allowed; use an object." ->
            TypeclassDiagnostic.InvalidInstanceCase.ClassBased()
        reason == "extension instance functions are not allowed." ->
            TypeclassDiagnostic.InvalidInstanceCase.ExtensionFunction()
        reason == "instance functions cannot declare a regular parameter." ->
            TypeclassDiagnostic.InvalidInstanceCase.RegularParameter()
        reason == "suspend instance functions are not allowed." ->
            TypeclassDiagnostic.InvalidInstanceCase.SuspendFunction()
        reason == "extension instance properties are not allowed." ->
            TypeclassDiagnostic.InvalidInstanceCase.ExtensionProperty()
        reason == "lateinit instance property declarations are not allowed." ->
            TypeclassDiagnostic.InvalidInstanceCase.LateinitProperty()
        reason == "mutable instance property declarations are not allowed." ->
            TypeclassDiagnostic.InvalidInstanceCase.MutableProperty()
        reason == "custom getter instance property declarations are not allowed." ->
            TypeclassDiagnostic.InvalidInstanceCase.CustomGetterProperty()
        reason ==
            "non-@Typeclass intermediate supertypes cannot provide inherited typeclass instances." ->
            TypeclassDiagnostic.InvalidInstanceCase.NonTypeclassSupertypes()
        reason == "@Instance declarations must provide a @Typeclass type." ->
            TypeclassDiagnostic.InvalidInstanceCase.MustProvideTypeclassType()
        reason ==
            "instance declarations must be top-level or live in a companion of the provided typeclass head or one of its associated owners." ->
            TypeclassDiagnostic.InvalidInstanceCase.WrongScopeCompanion()
        reason ==
            "associated owner does not match the provided typeclass head or its type arguments." ->
            TypeclassDiagnostic.InvalidInstanceCase.AssociatedOwnerMismatch()
        reason == "instance function context parameters must be typeclass prerequisites." ->
            TypeclassDiagnostic.InvalidInstanceCase.NonTypeclassPrerequisites()
        reason == "instance function typeclass prerequisites must not use star projections." ->
            TypeclassDiagnostic.InvalidInstanceCase.StarProjectedPrerequisites()
        reason ==
            "instance function typeclass prerequisites must not use definitely-non-null type arguments." ->
            TypeclassDiagnostic.InvalidInstanceCase.DefinitelyNotNullPrerequisites()
        ...
        typeParameterPrereqRegex.matches(reason) ->
            TypeclassDiagnostic.InvalidInstanceCase.TypeParameterOnlyInPrerequisites(
                typeParameterPrereqRegex.matchEntire(reason)!!.groupValues[1]
            )
        directRecursiveRegex.matches(reason) ->
            TypeclassDiagnostic.InvalidInstanceCase.DirectRecursive(
                directRecursiveRegex.matchEntire(reason)!!.groupValues[1]
            )
        else ->
            TypeclassDiagnostic.InvalidInstanceCase.Generic(
                reason.ensureSentence(),
                fix.ensureSentence(),
            )
    }

private fun parseInvalidEquivCase(
    reason: String,
    fix: String,
): TypeclassDiagnostic.InvalidEquivCase =
    when (reason.ensureSentence()) {
        "Equiv is compiler-owned; users must not subclass it directly." ->
            TypeclassDiagnostic.InvalidEquivCase.Subclassing()
        "Equiv is compiler-owned and cannot be published as an @Instance." ->
            TypeclassDiagnostic.InvalidEquivCase.PublishedInstance()
        else ->
            TypeclassDiagnostic.InvalidEquivCase.Generic(
                reason.ensureSentence(),
                fix.ensureSentence(),
            )
    }
...
private fun inferDiagnosticIdFromHeadline(headline: String): String? =
    when {
        ...
        headline == "Invalid builtin evidence request." ->
            TypeclassDiagnosticIds.INVALID_BUILTIN_EVIDENCE
        headline.startsWith("No context argument for required instance ") ->
            TypeclassDiagnosticIds.NO_CONTEXT_ARGUMENT
        headline.startsWith("Ambiguous typeclass instance for ") ->
            TypeclassDiagnosticIds.AMBIGUOUS_INSTANCE
        else -> null
    }

private data class ParsedNarrativeSections(
    val headline: String,
    val reason: String,
    val fix: String,
)
...
private fun fallbackCannotDeriveFix(reason: String): String =
    when {
        ...
        reason.contains("Cannot derive via", ignoreCase = true) ||
            reason.contains("disconnected", ignoreCase = true) ||
            reason.contains("ambiguous", ignoreCase = true) ->
            "adjust the DeriveVia path so every segment attaches uniquely from the annotated target to the requested terminal type."
        reason.contains("result head", ignoreCase = true) ||
            reason.contains("admissible", ignoreCase = true) ||
            reason.contains("not quantified", ignoreCase = true) ->
...
private fun String.ensureSentence(): String =
    trim().trimEnd().let { text -> if (text.endsWith('.')) text else "$text." }
