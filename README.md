...
K2 compiler plugin for Kotlin 2.3.10 that adds:
...
Both modules target Kotlin `2.3.10` and expect `-Xcontext-parameters`.
...
kotlinc \
  -Xcontext-parameters \
  -Xplugin="$HOME/.m2/repository/one/wabbit/kotlin-typeclasses-plugin/0.0.1/kotlin-typeclasses-plugin-0.0.1.jar" \
  ...
```
